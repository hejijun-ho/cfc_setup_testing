...
# Remove directories and fstab entries
sudo rm -rf /nix
sudo rm -rf /mydata/nix
# Correctly escape the path for sed
...
echo "🚀 使用 Nix 安裝 direnv, nix-direnv, bazelisk, grpcurl..."
nix profile install nixpkgs#direnv nixpkgs#nix-direnv nixpkgs#bazelisk nixpkgs#grpcurl
...
echo "🚀 建置 Ledger binary..."
./scripts/build.sh release
cd ..

# Clone Federated Compute (contains protobuf definitions for grpcurl)
if [ ! -d "federated-compute" ]; then
    echo "🚀 Clone Federated Compute..."
    git clone https://github.com/google-parfait/federated-compute.git
fi
cd federated-compute
echo "✅ Federated Compute 已 clone，不需額外建置。"
cd ..

# Clone Trusted Computations Platform
if [ ! -d "trusted-computations-platform" ]; then
    echo "🚀 Clone Trusted Computations Platform..."
    git clone https://github.com/google-parfait/trusted-computations-platform.git
fi
cd trusted-computations-platform
echo "✅ Trusted Computations Platform 已 clone，不需額外建置。"
cd ..
