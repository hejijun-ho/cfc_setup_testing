                    ...
                    match stream {
                        Ok(mut stream) => {
                            ...
                            match stream.read_to_end(&mut buffer) {
                                Ok(size) => {
                                    if size > 0 {
                                        info!("Read {} bytes from TCP stream, forwarding to guest.", size);
                                        ...
                                        if let Err(e) =
                                            oak_channel::basic_framed::send_raw(&mut host_socket_clone, &buffer)
                                        {
                                            log::error!("Failed to forward message to guest: {:?}", e);
                                        }
                                    }
                                }
                                Err(e) => {
